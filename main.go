...
import (
	...
	"github.com/Portshift/klar/client"
...
func exit(code int, conf *client.Config, scanResults *forwarding.ImageVulnerabilities) {
...
func main() {
	...
	conf, err := client.NewConfig(imageName)
	if err != nil {
		log.Errorf("Invalid options: %v", err)
		os.Exit(2)
	}

	vulnerabilities, err := client.ExecuteScan(conf)
	if err != nil {
...
func initLogs() {
	if os.Getenv(client.OptionKlarTrace) == "true" {
